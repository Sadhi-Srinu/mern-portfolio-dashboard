...
export const addNewTimeline = (timelineData) => async (dispatch) => {
  ...
  try {
    const {data} = await axios.post(
      "https://mern-portfolio-backend-vilj.onrender.com/api/v1/timeline/add", timelineData,
      {
        withCredentials: true,
...
export const deleteTimeline = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://mern-portfolio-backend-vilj.onrender.com/api/v1/timeline/delete/${id}`,
      {
        withCredentials: true,
      }
    );
